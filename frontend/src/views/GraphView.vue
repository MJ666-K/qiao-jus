...
const selectedEntity = computed(() =>
  selectedEntityId.value
    ? (entityById.value.get(selectedEntityId.value) ?? null)
    : null,
);
...
const entityRelations = computed(() => {
  if (!selectedEntityId.value)
    return { out: [] as GraphEdge[], in: [] as GraphEdge[] };
...
onMounted(async () => {
  ...
  await kb.loadDatasets();
  await loadDatasetGraph({ silent: true });
});

watch(
  () => kb.selectedDatasetId,
  (id, prev) => {
    if (id && id !== prev) void loadDatasetGraph({ silent: true });
  },
);

async function loadDatasetGraph(options?: { silent?: boolean }) {
  if (!loggedIn.value) {
  ...
  try {
    const r = await datasetGraph(kb.selectedDatasetId);
    entities.value = [...r.entities];
    relations.value = [...r.relations];
    lastEmpty.value = entities.value.length === 0;
    if (lastEmpty.value && !options?.silent) {
      notifyInfo("当前知识库暂无图谱", "文档索引完成后将自动生成实体关系");
    } else if (!options?.silent && !lastEmpty.value) {
      notifySuccess(
        "已加载知识库图谱",
        `${entities.value.length} 个实体 · ${relations.value.length} 条关系`,
      );
    }
  } catch (e) {
    notifyError("图谱加载失败", e instanceof Error ? e.message : undefined);
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div>
    ...
    <div class="toolbar-row card-panel">
      <DatasetScopeSelect
        width="220px"
        @change="() => loadDatasetGraph({ silent: true })"
      />
      <el-input
        v-model="query"
        placeholder="输入问题或实体名，局部检索"
        class="grow"
        clearable
        @keyup.enter="loadLocal"
      />
      <el-input-number v-model="depth" :min="1" :max="3" />
      <el-button type="primary" :loading="loading" @click="loadLocal"
        >局部检索</el-button
      >
      <el-button @click="loadDatasetGraph()">刷新全库</el-button>
    ...
    <div class="graph-layout">
      <div class="card-panel graph-main">
        <div class="canvas-toolbar">
          ...
          <el-button size="small" @click="canvasRef?.resetZoom()"
            >重置</el-button
          >
          <el-button
            size="small"
      ...
      <div class="drawer-content">
        <template v-if="drawerMode === 'entity' && selectedEntity">
          ...
          <h5 class="detail-section">
            出边 ({{ entityRelations.out.length }})
          </h5>
          <el-empty
            v-if="!entityRelations.out.length"
            description="暂无出边"
            :image-size="48"
          />
          <ul v-else class="rel-mini">
            <li
              v-for="r in entityRelations.out"
              :key="'o' + relationKey(r)"
          ...
          <el-empty
            v-if="!entityRelations.in.length"
            description="暂无入边"
            :image-size="48"
          />
          <ul v-else class="rel-mini">
            <li
              v-for="r in entityRelations.in"
              :key="'i' + relationKey(r)"
              @click="openRelation(r)"
            >
        ...
        <template v-else-if="drawerMode === 'relation' && selectedRelation">
          <div class="relation-flow">
            <div class="node">
              ...
              <div
                class="node-name"
                @click="openEntity(selectedRelation.source)"
              >
            ...
            <div class="node">
              ...
              <div
                class="node-name"
                @click="openEntity(selectedRelation.target)"
              >
          ...
          <div class="rel-tags">
            <span class="rel-tag type">{{
              selectedRelation.type || "RELATED"
            }}</span>
            <span
              class="rel-tag desc"
              v-if="selectedRelation.description?.trim()"
            >
...
.relation-flow {
  ...
  gap: 12px;
  padding: 14px;
  background: linear-gradient(135deg, #f8fafc 0%, #fafaf9 100%);
  border-radius: 10px;
  margin-bottom: 16px;
}
.relation-flow .node {
...
.relation-flow .node-name {
  font-size: 13px;
  font-weight: 500;
  color: #1e293b;
  cursor: pointer;
}
.relation-flow .node-name:hover {
  color: #f97316;
  text-decoration: underline;
}
.relation-flow .node-label {
  font-size: 10px;
  color: #94a3b8;
  margin-top: 4px;
}
